<template>
  <div class="relative border-r border-gray-200">
    <button @click="toggleDropdown" :class="{'bg-my-teal text-white hover:bg-teal-600 border border-teal-600 rounded-r-md ': selectedTags.length > 0, 'text-gray-700 hover:bg-gray-100 rounded-r-md': selectedTags.length === 0}" class="flex items-center gap-1 px-3 py-2 text-sm font-medium">
      Filter
    ...
    <div v-if="isOpen" class="absolute right-0 z-50 mt-1 w-60 divide-y divide-gray-100 rounded-md border border-gray-300 bg-white shadow-md p-2" role="menu">
      ...
      <div class="mb-2 flex items-center justify-between">
        <input v-model="searchTerm" type="text" class="w-full rounded-md border border-gray-300 px-2 py-1 text-sm text-gray-700 focus:outline-none" placeholder="Search tags...">
